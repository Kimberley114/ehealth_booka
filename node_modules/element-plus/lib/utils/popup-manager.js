...
const getModal = function () {
    ...
    if (modalDom) {
        ...
        dom_1.on(modalDom, 'touchmove', onTouchMove);
        dom_1.on(modalDom, 'click', onModalClick);
    }
    return modalDom;
};
const instances = {};
const PopupManager = {
    ...
    openModal: function (id, zIndex, dom, modalClass, modalFade) {
        ...
        dom_1.addClass(modalDom, 'v-modal');
        if (this.modalFade && !hasModal) {
            dom_1.addClass(modalDom, 'v-modal-enter');
        }
        if (modalClass) {
            const classArr = modalClass.trim().split(/\s+/);
            classArr.forEach(item => dom_1.addClass(modalDom, item));
        }
        setTimeout(() => {
            dom_1.removeClass(modalDom, 'v-modal-enter');
        }, 200);
    ...
    closeModal: function (id) {
        ...
        if (modalStack.length > 0) {
            ...
            if (topItem.id === id) {
                if (topItem.modalClass) {
                    ...
                    classArr.forEach(item => dom_1.removeClass(modalDom, item));
                }
                modalStack.pop();
                if (modalStack.length > 0) {
        ...
        if (modalStack.length === 0) {
            if (this.modalFade) {
                dom_1.addClass(modalDom, 'v-modal-leave');
            }
            setTimeout(() => {
                if (modalStack.length === 0) {
                ...
                dom_1.removeClass(modalDom, 'v-modal-leave');
            }, 200);
        }
    },
};
...
if (!isServer_1.default) {
    ...
    dom_1.on(window, 'keydown', function (event) {
